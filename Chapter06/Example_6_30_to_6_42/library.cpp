...
{
    return QString("%1%2%3%4%5%6%7").arg(m_ItemType).arg(sep).arg(m_ISBN).arg(sep)
...
ReferenceBook::ReferenceBook(QString type, QString isbn, QString title, QString author,
                             QString pub, int year, RefCategory refcat, int numCopies)
...
Dvd::Dvd(QString type, QString isbn, QString title, int disks, bool twoSided, int numCopies)
    : RefItem(type, isbn, title,  numCopies), m_NumberOfDisks(disks), m_TwoSided(twoSided)
{
...
Dvd::Dvd(QStringList& plst)
    : RefItem(plst), m_NumberOfDisks(plst.takeFirst().toInt()),
...
Film::Film(QStringList& plst)
    : Dvd(plst), m_Star(plst.takeFirst()), m_Director(plst.takeFirst()),
      m_Minutes(plst.takeFirst().toInt()), m_BlueRay(plst.takeFirst().toInt())
{
...
void Library::addRefItem(RefItem*& refitem)
{
    ...
    RefItem* oldItem(findReftItem(isbn));
    if(oldItem == 0)
        append(refitem);
    else
    {
...
int Library::removeReftItem(QString isbn)
{
    RefItem* ref(findReftItem(isbn));
    int numCopies(-1);
    if(ref)
    {
